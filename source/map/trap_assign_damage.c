...
{
    sfFloatRect player_rect = {player->pos.x, player->pos.y, 14.0, 10.0};

    sfVector2i top_right_tile =
    {floor((player_rect.left + player_rect.width) / 16),
    floor((player_rect.top + player_rect.height) / 16)};
    sfVector2i bottom_left_tile = {floor((player_rect.left) / 16),
    floor((player_rect.top + player_rect.height) / 16)};
...
void collision_traps(entity_t *player, layer_t *layers)
{
    ...
    {floor(player_rect.left / 16), floor(player_rect.top / 16)};
    sfVector2i bottom_right_tile =
    {floor((player_rect.left + player_rect.width)
    / 16), floor((player_rect.top) / 16)};
